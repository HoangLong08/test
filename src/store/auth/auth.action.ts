...
const doSendMailForgotPasswordAction = createAsyncThunk(
  ...
  async (
    ...
    try {
      const res = await auth.doSendMailForgotPassword({
        email,
        languageCode,
      });
      return { ...res, email };
...
const verifyTokenActivationAccountAction = createAsyncThunk(
  'auth/verifyTokenActivationAccountAction',
  async (token: string, thunkAPI) => {
    try {
      const res = await auth.verifyTokenActivationAccount(token);
      return res;
    } catch (error: any) {
      return thunkAPI.rejectWithValue({ error: error.response.data.message });
    }
