...
import {
  ...
  verifyTokenActivationAccountAction,
} from './auth.action';
import {
  AuthState,
  FormAddressType,
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // ---------------
      .addCase(verifyTokenActivationAccountAction.pending, (state: AuthState) => ({
        ...state,
        token: {
          expiresIn: 0,
          accessToken: '',
      ...
      .addCase(verifyTokenActivationAccountAction.fulfilled, (state: AuthState, action: any) => {
        const { data } = action.payload;
        localStorage.setItem(
          'persist:auth-yaly',
          JSON.stringify({
      ...
      .addCase(verifyTokenActivationAccountAction.rejected, (state: AuthState) =>
      ...
      .addCase(postAddressWithUserAction.fulfilled, (state: AuthState, action: any) => {
        ...
        return {
          ...state,
          myAddress: {
            data: [dataResponse, ...state.myAddress.data],
            loading: false,
          },
        };
      })
